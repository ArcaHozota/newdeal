...
func PaginationHymns(keyword string, pageSize uint8, offset uint32) ([]models.Hymn, error) {
	...
	err := models.DB.Model(&models.Hymn{}).InnerJoins("hymns_work on hymns_work.work_id = hymns.id").Where(&models.Hymn{NameJp: searchStr}).Or(&models.Hymn{NameKr: searchStr}).Or(&models.Hymn{HymnWork: models.HymnWork{NameJpRa: searchStr}}).Limit(int(pageSize)).Offset(int(offset)).Find(&hymns).Error
	return hymns, err
}
