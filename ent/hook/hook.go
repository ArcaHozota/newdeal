...
// Mutate calls f(ctx, m).
func (f HymnFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HymnMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.HymnMutation", m)
}

// The HymnsWorkFunc type is an adapter to allow the use of ordinary
// function as HymnsWork mutator.
type HymnsWorkFunc func(context.Context, *ent.HymnsWorkMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f HymnsWorkFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HymnsWorkMutation); ok {
		return f(ctx, mv)
	}
