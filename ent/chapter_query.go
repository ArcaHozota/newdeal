...
type ChapterQuery struct {
	...
	withToPhrase    *PhraseQuery
	withChapterBook *BookQuery
...
// QueryToPhrase chains the current query on the "to_phrase" edge.
func (cq *ChapterQuery) QueryToPhrase() *PhraseQuery {
	query := (&PhraseClient{config: cq.config}).Query()
...
// QueryChapterBook chains the current query on the "chapter_book" edge.
func (cq *ChapterQuery) QueryChapterBook() *BookQuery {
...
func (cq *ChapterQuery) FirstID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (cq *ChapterQuery) FirstIDX(ctx context.Context) int32 {
...
func (cq *ChapterQuery) OnlyID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (cq *ChapterQuery) OnlyIDX(ctx context.Context) int32 {
...
func (cq *ChapterQuery) IDs(ctx context.Context) (ids []int32, err error) {
...
func (cq *ChapterQuery) IDsX(ctx context.Context) []int32 {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *ChapterQuery) Clone() *ChapterQuery {
	...
	return &ChapterQuery{
		...
		withToPhrase:    cq.withToPhrase.Clone(),
		withChapterBook: cq.withChapterBook.Clone(),
...
// WithToPhrase tells the query-builder to eager-load the nodes that are connected to
// the "to_phrase" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *ChapterQuery) WithToPhrase(opts ...func(*PhraseQuery)) *ChapterQuery {
	query := (&PhraseClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withToPhrase = query
	return cq
}

// WithChapterBook tells the query-builder to eager-load the nodes that are connected to
// the "chapter_book" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *ChapterQuery) WithChapterBook(opts ...func(*BookQuery)) *ChapterQuery {
	query := (&BookClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withChapterBook = query
	return cq
}
...
func (cq *ChapterQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chapter, error) {
	...
	if query := cq.withToPhrase; query != nil {
		if err := cq.loadToPhrase(ctx, query, nodes,
			func(n *Chapter) { n.Edges.ToPhrase = []*Phrase{} },
			func(n *Chapter, e *Phrase) { n.Edges.ToPhrase = append(n.Edges.ToPhrase, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withChapterBook; query != nil {
		if err := cq.loadChapterBook(ctx, query, nodes, nil,
			func(n *Chapter, e *Book) { n.Edges.ChapterBook = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChapterQuery) loadToPhrase(ctx context.Context, query *PhraseQuery, nodes []*Chapter, init func(*Chapter), assign func(*Chapter, *Phrase)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int32]*Chapter)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(phrase.FieldChapterID)
	}
	query.Where(predicate.Phrase(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chapter.ToPhraseColumn), fks...))
	}))
...
func (cq *ChapterQuery) loadChapterBook(ctx context.Context, query *BookQuery, nodes []*Chapter, init func(*Chapter), assign func(*Chapter, *Book)) error {
	ids := make([]int16, 0, len(nodes))
	nodeids := make(map[int16][]*Chapter)
	for i := range nodes {
		fk := nodes[i].BookID
...
func (cq *ChapterQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(chapter.Table, chapter.Columns, sqlgraph.NewFieldSpec(chapter.FieldID, field.TypeInt32))
	_spec.From = cq.sql
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withChapterBook != nil {
			_spec.Node.AddColumnOnce(chapter.FieldBookID)
		}
	}
