...
// QueryStudentRole chains the current query on the "student_role" edge.
func (sq *StudentQuery) QueryStudentRole() *RoleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, student.StudentRoleTable, student.StudentRoleColumn),
		)
		fromU = sqlgraph.SetNeighbors(sq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (sq *StudentQuery) Clone() *StudentQuery {
	...
	return &StudentQuery{
		...
		withStudentRole:  sq.withStudentRole.Clone(),
...
// WithStudentRole tells the query-builder to eager-load the nodes that are connected to
// the "student_role" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *StudentQuery) WithStudentRole(opts ...func(*RoleQuery)) *StudentQuery {
	query := (&RoleClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withStudentRole = query
	return sq
}
...
//		GroupBy(student.FieldLoginAccount).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Student.Query().
//		Select(student.FieldLoginAccount).
//		Scan(ctx, &v)
...
func (sq *StudentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Student, error) {
	...
	if query := sq.withStudentRole; query != nil {
		if err := sq.loadStudentRole(ctx, query, nodes, nil,
			func(n *Student, e *Role) { n.Edges.StudentRole = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StudentQuery) loadStudentRole(ctx context.Context, query *RoleQuery, nodes []*Student, init func(*Student), assign func(*Student, *Role)) error {
