...
// QueryToChapter chains the current query on the "to_chapter" edge.
func (bq *BookQuery) QueryToChapter() *ChapterQuery {
...
func (bq *BookQuery) FirstID(ctx context.Context) (id int16, err error) {
	var ids []int16
...
func (bq *BookQuery) FirstIDX(ctx context.Context) int16 {
...
func (bq *BookQuery) OnlyID(ctx context.Context) (id int16, err error) {
	var ids []int16
...
func (bq *BookQuery) OnlyIDX(ctx context.Context) int16 {
...
func (bq *BookQuery) IDs(ctx context.Context) (ids []int16, err error) {
...
func (bq *BookQuery) IDsX(ctx context.Context) []int16 {
	ids, err := bq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (bq *BookQuery) Clone() *BookQuery {
	...
	return &BookQuery{
		...
		withToChapter: bq.withToChapter.Clone(),
...
// WithToChapter tells the query-builder to eager-load the nodes that are connected to
// the "to_chapter" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BookQuery) WithToChapter(opts ...func(*ChapterQuery)) *BookQuery {
	query := (&ChapterClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withToChapter = query
	return bq
}
...
func (bq *BookQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Book, error) {
	...
	if query := bq.withToChapter; query != nil {
		if err := bq.loadToChapter(ctx, query, nodes,
			func(n *Book) { n.Edges.ToChapter = []*Chapter{} },
			func(n *Book, e *Chapter) { n.Edges.ToChapter = append(n.Edges.ToChapter, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BookQuery) loadToChapter(ctx context.Context, query *ChapterQuery, nodes []*Book, init func(*Book), assign func(*Book, *Chapter)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int16]*Book)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(chapter.FieldBookID)
	}
	query.Where(predicate.Chapter(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(book.ToChapterColumn), fks...))
	}))
...
func (bq *BookQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(book.Table, book.Columns, sqlgraph.NewFieldSpec(book.FieldID, field.TypeInt16))
	_spec.From = bq.sql
