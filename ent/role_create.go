...
func (rc *RoleCreate) SetVisibleFlg(b bool) *RoleCreate {
	rc.mutation.SetVisibleFlg(b)
	return rc
}
...
func (rc *RoleCreate) AddRoledIDs(ids ...int64) *RoleCreate {
	rc.mutation.AddRoledIDs(ids...)
	return rc
}

// AddRoled adds the "roled" edges to the Student entity.
func (rc *RoleCreate) AddRoled(s ...*Student) *RoleCreate {
	ids := make([]int64, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return rc.AddRoledIDs(ids...)
}
...
func (rc *RoleCreate) AddAuth(a ...*Auth) *RoleCreate {
...
func (rc *RoleCreate) check() error {
	...
	if _, ok := rc.mutation.VisibleFlg(); !ok {
		return &ValidationError{Name: "visible_flg", err: errors.New(`ent: missing required field "Role.visible_flg"`)}
	}
	return nil
}
...
func (rc *RoleCreate) createSpec() (*Role, *sqlgraph.CreateSpec) {
	...
	if value, ok := rc.mutation.VisibleFlg(); ok {
		_spec.SetField(role.FieldVisibleFlg, field.TypeBool, value)
		_node.VisibleFlg = value
	}
	if nodes := rc.mutation.RoledIDs(); len(nodes) > 0 {
	...
	if nodes := rc.mutation.AuthIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
