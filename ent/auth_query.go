...
// QueryToRoles chains the current query on the "to_roles" edge.
func (aq *AuthQuery) QueryToRoles() *RoleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, auth.ToRolesTable, auth.ToRolesPrimaryKey...),
		)
...
func (aq *AuthQuery) FirstID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (aq *AuthQuery) FirstIDX(ctx context.Context) int64 {
...
func (aq *AuthQuery) OnlyID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (aq *AuthQuery) OnlyIDX(ctx context.Context) int64 {
...
func (aq *AuthQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (aq *AuthQuery) IDsX(ctx context.Context) []int64 {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *AuthQuery) Clone() *AuthQuery {
	...
	return &AuthQuery{
		...
		withToRoles: aq.withToRoles.Clone(),
...
// WithToRoles tells the query-builder to eager-load the nodes that are connected to
// the "to_roles" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AuthQuery) WithToRoles(opts ...func(*RoleQuery)) *AuthQuery {
	query := (&RoleClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withToRoles = query
	return aq
}
...
func (aq *AuthQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Auth, error) {
	...
	if query := aq.withToRoles; query != nil {
		if err := aq.loadToRoles(ctx, query, nodes,
			func(n *Auth) { n.Edges.ToRoles = []*Role{} },
			func(n *Auth, e *Role) { n.Edges.ToRoles = append(n.Edges.ToRoles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AuthQuery) loadToRoles(ctx context.Context, query *RoleQuery, nodes []*Auth, init func(*Auth), assign func(*Auth, *Role)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Auth)
	nids := make(map[int64]map[*Auth]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(auth.ToRolesTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(auth.ToRolesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(auth.ToRolesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(auth.ToRolesPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Auth]struct{}{byID[outValue]: {}}
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "to_roles" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (aq *AuthQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(auth.Table, auth.Columns, sqlgraph.NewFieldSpec(auth.FieldID, field.TypeInt64))
	_spec.From = aq.sql
