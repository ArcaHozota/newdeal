...
func (aq *AuthQuery) QueryRoles() *RoleQuery {
...
func (aq *AuthQuery) WithRoles(opts ...func(*RoleQuery)) *AuthQuery {
	query := (&RoleClient{config: aq.config}).Query()
...
func (aq *AuthQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Auth, error) {
	...
	if query := aq.withRoles; query != nil {
		if err := aq.loadRoles(ctx, query, nodes,
			func(n *Auth) { n.Edges.Roles = []*Role{} },
			func(n *Auth, e *Role) { n.Edges.Roles = append(n.Edges.Roles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AuthQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*Auth, init func(*Auth), assign func(*Auth, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(auth.RolesTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(auth.RolesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(auth.RolesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(auth.RolesPrimaryKey[1]))
