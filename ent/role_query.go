...
// QueryRoled chains the current query on the "roled" edge.
func (rq *RoleQuery) QueryRoled() *StudentQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, role.RoledTable, role.RoledColumn),
		)
...
func (rq *RoleQuery) QueryAuth() *AuthQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, role.AuthTable, role.AuthPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(rq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withRoled:  rq.withRoled.Clone(),
		withAuth:   rq.withAuth.Clone(),
...
// WithRoled tells the query-builder to eager-load the nodes that are connected to
// the "roled" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoleQuery) WithRoled(opts ...func(*StudentQuery)) *RoleQuery {
	query := (&StudentClient{config: rq.config}).Query()
...
func (rq *RoleQuery) WithAuth(opts ...func(*AuthQuery)) *RoleQuery {
	query := (&AuthClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withRoled; query != nil {
		if err := rq.loadRoled(ctx, query, nodes,
			func(n *Role) { n.Edges.Roled = []*Student{} },
			func(n *Role, e *Student) { n.Edges.Roled = append(n.Edges.Roled, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withAuth; query != nil {
		if err := rq.loadAuth(ctx, query, nodes,
			func(n *Role) { n.Edges.Auth = []*Auth{} },
			func(n *Role, e *Auth) { n.Edges.Auth = append(n.Edges.Auth, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadRoled(ctx context.Context, query *StudentQuery, nodes []*Role, init func(*Role), assign func(*Role, *Student)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(student.FieldRoleID)
	}
	query.Where(predicate.Student(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.RoledColumn), fks...))
	}))
...
func (rq *RoleQuery) loadAuth(ctx context.Context, query *AuthQuery, nodes []*Role, init func(*Role), assign func(*Role, *Auth)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.AuthTable)
		s.Join(joinT).On(s.C(auth.FieldID), joinT.C(role.AuthPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(role.AuthPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.AuthPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "auth" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
